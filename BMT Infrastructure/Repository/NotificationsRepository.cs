...
namespace com.blazor.bmt.infrastructure.repositories
{
    public class NotificationsRepository : RepositoryTransaction<Notification>,  INotificationsRepository
    {
        ...
        {
            return await _dbContext.Notifications.AsNoTracking()
                .Where(x => x.Id == (model.Id == 0 ? x.Id : model.Id) && x.CreatedBy == (model.CreatedBy == 0 ? x.CreatedBy : model.CreatedBy) && x.OrganizationId == (model.OrganizationId == 0 ? x.OrganizationId : model.OrganizationId) && x.Status == (model.Status == 0 ? x.Status : model.Status) && x.NetworkId==(model.NetworkId==0? x.NetworkId: model.NetworkId) && (string.IsNullOrEmpty(model.MessageRefId) || (""+x.MessageRefId).Contains(model.MessageRefId)) && (string.IsNullOrEmpty(model.Recipient) || ("" + x.Recipient).Contains(model.Recipient)))// && Convert.ToDateTime(x.CreatedAt) <= (model.CreatedAt.Year<=1900 ? Convert.ToDateTime(x.CreatedAt) : Convert.ToDateTime(model.CreatedAt)))
                .ToListAsync();
        }
        public async Task<IEnumerable<Notification>> GetNotificationsLogAllFiltersAsync(Notification model)
        {
            return await _dbContext.Notifications.AsNoTracking()
                .Where(x => x.Id == (model.Id == 0 ? x.Id : model.Id) && x.CreatedBy == (model.CreatedBy == 0 ? x.CreatedBy : model.CreatedBy) && x.OrganizationId == (model.OrganizationId == 0 ? x.OrganizationId : model.OrganizationId) && x.DeliveryStatus == (model.DeliveryStatus == 0 ? x.DeliveryStatus : model.DeliveryStatus) && x.NetworkId == (model.NetworkId == 0 ? x.NetworkId : model.NetworkId) && (string.IsNullOrEmpty(model.MessageRefId) || ("" + x.MessageRefId).Contains(model.MessageRefId)) && (string.IsNullOrEmpty(model.Recipient) || ("" + x.Recipient).Contains(model.Recipient)) && Convert.ToDateTime(x.CreatedAt) >= (model.CreatedAt.Year<=1900 ? Convert.ToDateTime(x.CreatedAt) : Convert.ToDateTime(model.CreatedAt)) && Convert.ToDateTime(x.LastUpdatedAt) >= (model.LastUpdatedAt == null ? Convert.ToDateTime(x.LastUpdatedAt) : Convert.ToDateTime(model.LastUpdatedAt)))
                .ToListAsync();
        }
    }
}
