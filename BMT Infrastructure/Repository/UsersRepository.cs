...
namespace com.blazor.bmt.infrastructure.repositories
{
    public class UsersRepository : Repository<User>, IUsersRepository
    {
        ...
        public async Task<IEnumerable<User>> GetDspMobileUsersAllFiltersAsync(User model) {
            return await _dbContext.Users.AsNoTracking()
                .Where(x => x.Id == (model.Id == 0 ? x.Id : model.Id) && x.Dspid == (Convert.ToInt32(model.Dspid) == 0 ? x.Dspid : model.Dspid) && x.RoleId == (int)UTIL.USER_ROLES.DA && x.Status == (model.Status == 0 ? x.Status : model.Status) && x.Fmctoken != null && ("" +x.Fmctoken).Length > 5 )
                .ToListAsync();
        }
        ...
        {
            return await _dbContext.Users.AsNoTracking()
                .Where(x => x.RoleId== roleId && (x.Dspid==(dspId==0? x.Dspid: dspId))).OrderBy(x => x.FirstName)
                .ToListAsync();
        }
    }
}
