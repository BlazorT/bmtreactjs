...
const DASafetyTab = () => {
  ...
  return (
    <CContainer fluid className="mt-3">
      ...
      <CRow>
        <CollapseHeader
          title="Metric Event-Level Detail"
          isVisible={matricEventVisible}
          toggleVisible={() => setMatricEventVisible(!matricEventVisible)}
        />
        <CCollapse visible={matricEventVisible} className="mt-3">
          <CRow>
            <CCol md={12}>
              <StatusMetricCard val={0} attributesStatus2={[{ title: 'NO DATA', value: 0 }]} />
            </CCol>
          </CRow>
        </CCollapse>
      </CRow>
      <CRow className="mt-3">
        ...
        />
        <SafetyMatricTab isCollapse={matricDeepVisible} />
      </CRow>
    </CContainer>
  );
};
export default DASafetyTab;
