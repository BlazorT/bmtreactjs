...
export const getRosterPdf = (reportRows, reportField) => {
  ...
  doc.setFontSize(13);
  doc.setFont('times', 'bold');
  doc.text('Delivery Associate (DA) dispatchment Report (DADR)', 5, 40);
  ...
  // Explanation Text
  doc.setFontSize(10);
  const labelY = 20 + additionalInfo.length * 2 + 5;
  ...
  // Table Headers
  const data = [
    ['Roster Date', 'DA', 'Start Week', 'End Week', 'Vehicle'],

    ['01/16/2024', '5', '','','10'],
    ['', 'Basedine, 089029', 'Sun, Jan 28', 'Sat, Feb 03', 'Toyota Grandee, U8078'],
    ['', 'Geraldine, 089029', 'Sun, Jan 28', 'Sat, Feb 03','Toyota Gli,U87998'],
    ['02/01/2024', '8', '','','12'],
    ['', 'palisten, 837009', 'Sun, Jan 28, Sedan', 'Sat, Feb 03', 'Toyota Corolla,U998'],
    ['', 'dinewarm, 834499', 'Sun, Jan 28', 'Sat, Feb 03', 'Toyota Revo,U878'],
  ];
  ...
  const columnWidths = [35, 45, 55,30,40];
  ...
  data.forEach((row) => {
    ...
    if (isGroup) {
      ...
      doc.setLineWidth(0.005);
      doc.setFont('times', 'bold');
      doc.rect(tableX, tableY + 5, fullWidth, 5, 'FD'); // Set the width to the whole page
