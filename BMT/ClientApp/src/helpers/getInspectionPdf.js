...
export const getInspectionPdf = (reportRows, reportField) => {
  ...
  doc.setFontSize(13);
  doc.setFont('times', 'bold');
  doc.text("Driver's Vehicle Inspection Report (DVIR)", 5, 40);
  ...
  const additionalInfo = [
    {
      key: 'Inspection #',
      value: reportField[0].id.toString() + Math.floor(100000 + Math.random() * 900000),
    },
    {
      key: 'Inspection Type',
      value: globalutil.inspectionstatuses().find((item) => item.id == reportField[0].reportTypeId)
        .name,
    },
    { key: 'Inspection Date', value: formatDateTime(reportField[0].createdAt) },
    { key: 'Driver/Inspector #', value: reportField[0].inspectorName },
    { key: 'DSP code', value: reportField[0].id.toString() },
    { key: 'Station', value: '' },
    { key: 'Location', value: '' },
  ];

  const addionalLeftInfo = [
    { key: 'Carrier', value: reportField[0].ownershipname },
    { key: 'Asset type', value: reportField[0].categoryname },
    { key: 'VIN', value: reportField[0].vehicleWinCode },
    { key: 'License plate', value: reportField[0].numberPlate },
    { key: 'Odometer', value: '-' },
  ];
  ...
  // Explanation Text
  doc.setFontSize(10);
  const labelY = 70 + additionalInfo.length * 2 + 5;
  ...
  let group = 1;
  data.forEach((row) => {
    const isVehicleSideNotEmpty = row[0] === 'Part and Accessories';
    const isGroup =
      row[0] === 'Front Side' ||
      row[0] === 'Back Side' ||
      row[0] === 'Driver Side' ||
    ...
    if (isGroup) {
      ...
      row.forEach((cell, index) => {
        ...
        if (cell === '4') {
          doc.text(cell, tableX + 7, tableY + 16);
        } else {
