...
namespace com.blazor.bmt.controllers
{
    ...
    { //Almas 
        ...
        public TemplateController(Microsoft.Extensions.Hosting.IHostingEnvironment hostingEnvironment, ITemplatevariableService templatevariableService, ICampaignTemplateService campaignTemplateService, IAppLogPageService appLogPageService, IHttpContextAccessor httpContextAccessor, ILogger<TemplateController> logger,  IMemoryCache cache)
        {
            _logger = logger;           
        ...
        [HttpPost("submitvariable")]
        [Route("submitvariable")]
        public async Task<ActionResult> SubmitVariableTemplateData([FromBody] TemplatevariableModel cm)
		{
			...
			try
			{
                if (cm.Id <= 0)
                {
                    blazorApiResponse.data = await _templatevariableService.Create(cm);
                    blazorApiResponse.status = true;
                    //GlobalUTIL.loadConfigurations(GlobalSettings.DefaultOrgId);
                    blazorApiResponse.message = string.Format(BlazorConstant.INSERTED_SUCCESS, cm.Name, GlobalUTIL.CurrentDateTime.ToLongDateString());
                }
                else {
				    await _templatevariableService.Update(cm);
                    blazorApiResponse.data = cm;
					blazorApiResponse.status = true;
					//GlobalUTIL.loadConfigurations(GlobalSettings.DefaultOrgId);
					blazorApiResponse.message = string.Format(BlazorConstant.UPDATED_SUCCESS, cm.Name, GlobalUTIL.CurrentDateTime.ToLongDateString());
        ...
        [HttpGet("getallvariablesdata")]
        [HttpPost("getallvariablesdata")]
        [Route("getallvariablesdata")]
        public async Task<ActionResult> GetAllVariables([FromBody] TemplatevariableModel filter)
        {
            if (string.IsNullOrWhiteSpace(Request.Headers["Authorization"]) || (Convert.ToString(Request.Headers["Authorization"]).Contains(BlazorConstant.API_AUTH_KEY) == false)) return Ok(new BlazorApiResponse { status = false, errorCode = "405", effectedRows = 0, data = "Authorization Failed" });
            BlazorApiResponse blazorApiResponse = new BlazorApiResponse();
            try
            {
                //var uvmr = UTIL.userls.Where(x => x.storeid == cvm.storeid && x.username == cvm.username && x.status == cvm.status && x.password == uvm.password);
                blazorApiResponse.status = true;
                blazorApiResponse.data = await _templatevariableService.GetTemplateVariablesAllFiltersAsync(filter);
            }
            catch (Exception ex)
            {
                blazorApiResponse.status = false;
